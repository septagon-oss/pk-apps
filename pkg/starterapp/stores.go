...
type moduleStores struct {
	...
	// branding is nil on the Postgres profile: pk-modules ships only the
	// SQLite adapter for branding_management today. BuildApp composes the
	// branding module only when this field is set, and refuses to boot if the
	// seed asks for branding on a profile that cannot store it — absence is
	// explicit at the one seam that owns engine choice, never a silent quirk.
	branding brandingstore.Store
}
...
func openModuleStores(db *sql.DB, driver string) (moduleStores, error) {
	...
	if isPostgres(driver) {
		...
		// No branding store: pk-modules has no Postgres branding adapter yet,
		// so s.branding stays nil and branding_management is not composed on
		// this profile (see moduleStores).
		return s, nil
	}
	...
	if s.branding, err = brandingsqlite.New(db); err != nil {
		return s, fmt.Errorf("branding store: %w", err)
	}
	return s, nil
}
